...
class BaseStatisticsRecord:
    ...
    def generate_individual_figures(
        self, summary_text: str = None, file_prefix: str = ""
    ):
        """
        Генерує окремі фігури для кожного графіка та для summary-тексту,
        і зберігає кожну картинку у файл.

        Створюються:
          - Фігура для графіка рівня стресу (файл "stress.png")
          - Фігура для графіка складності тренувань (файл "hardness.png")
          - Фігура для графіка годин сну (файл "sleep.png")
          - Фігура для графіка самопочуття (файл "feelings.png")
          - Фігура для summary‑тексту (файл "summary.png")

        Повертає словник з ключами: "stress", "hardness", "sleep", "feelings", "summary",
        де значення — це шляхи до збережених файлів.
        """
        stress_file = f"{file_prefix}_stress.png"
        hardness_file = f"{file_prefix}_hardness.png"
        feelings_file = f"{file_prefix}_feelings.png"
        sleep_file = f"{file_prefix}_sleep.png"
        summary_file = f"{file_prefix}_summary.png"
        ...
        fig_stress.savefig(stress_file, dpi=150)
        ...
        if not hardness_df.empty:
            ...
            if not soreness_df.empty:
                # Для кожного запису крепатури використовуємо значення "hardness" із hardness_df,
                # але оскільки дані можуть не збігатися за датою, відобразимо їх на рівні, що близький до максимальної складності.
                max_hardness = hardness_df["hardness"].max()
                ax_hardness.scatter(
                    soreness_df["dt"],
                    [max_hardness * 0.9] * len(soreness_df),
                    color="red",
                    marker="X",
                    s=100,
                )
        ...
        fig_hardness.savefig(hardness_file, dpi=150)
        ...
        fig_sleep.savefig(sleep_file, dpi=150)
        ...
        fig_feelings.savefig(feelings_file, dpi=150)

        # Фігура 5: Summary-текст
        ...
        fig_summary.savefig(summary_file, dpi=150)

        return {
            "stress": stress_file,
            "hardness": hardness_file,
            "sleep": sleep_file,
            "feelings": feelings_file,
...
if __name__ == "__main__":
    ...
    weekly_files = weekly_stats.generate_individual_figures(file_prefix="weekly")
    ...
    monthly_files = monthly_stats.generate_individual_figures(file_prefix="monthly")
    print("[LOG] Метрики (JSON):")
    print(monthly_stats.get_metrics_json())

    # Приклад виведення шляхів до файлів:
    print("Тижнева статистика збережена у файли:", weekly_files)
    print("Місячна статистика збережена у файли:", monthly_files)
